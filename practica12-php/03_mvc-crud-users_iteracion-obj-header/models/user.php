<?php
    ...
    class UsuariosDB extends ConectorDB{
        ...
        // public function ModificarEstadoDB($rowid){
        //     $sql = "SELECT `estado` FROM `app_usuarios` WHERE `rowid`= {$rowid}";
        //     $datos = $this->consultarDB($sql);
        //     foreach($datos as $dato);// de mysql result a Array
        //     ($dato['estado'] == 0) ? $estado = 1 : $estado = 0;
        //     $sql = "UPDATE `app_usuarios` SET `estado` = {$estado} WHERE `rowid` = {$rowid}";
        //     $res = $this->consultarDB($sql);
        //     return $res;
        // }
        ...
        public function ModificarConnectDB($rowid){
            $sql = "SELECT `conectado` FROM `app_usuarios` WHERE `rowid`={$rowid}";
            $datos = $this->consultarDB($sql);
            foreach($datos as $dato);
            ($dato['conectado'] == 0) ? $conectado = 1 : $conectado = 0;
            $sql = "UPDATE `app_usuarios` SET `conectado` = {$conectado} WHERE `rowid` = {$rowid}";
